...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    {
        NSString *pinyin = [((WChineseSting*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((WChineseSting*)object).pinYin substringToIndex:1];
        NSString *string = ((WChineseSting*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        WChineseSting *chineseString=[[WChineseSting alloc]init];
        ...
        chineseString.string =[WChineseSting RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        [result addObject:((WChineseSting*)[tempArray objectAtIndex:i]).string];
        NSLog(@"SortArray----->%@",((WChineseSting*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
