...
@interface YBFDCalendarItem () <UICollectionViewDataSource, UICollectionViewDelegate>
@property (nonatomic,assign) NSInteger selectIndex;
@end

@implementation YBFDCalendarItem

- (instancetype)init {
    if (self = [super init]) {
        self.backgroundColor = [UIColor clearColor];
        [self setupCollectionView];
        [self setFrame:CGRectMake(0, 0, DeviceWidth-30, YBFDCalendarH-topTitleH-ITEMHEIGTH)];
    }
    return self;
}
...
- (void)reloadData
{
    ...
    NSLog(@"self.date:%@",self.seletedDate);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < firstWeekday) {// 小于这个月的第一天
        ...
        // [self getCurrentData:indexPath] == self.seletedDate
        // self.selectIndex && self.selectIndex == indexPath.row
